import type{Camera,PtzDriver}from'./types.js';import{ViscaUdpDriver}from'./viscaUdp.js';import{ViscaTcpDriver}from'./viscaTcp.js';import{ViscaSerialDriver}from'./viscaSerial.js';import{NdiDriver}from'./ndi.js';
export function createDriver(c:Camera):PtzDriver{
 switch(c.protocol){
  ...
  case'ndi':if(!c.ndiSource)throw new Error('NDI source is required');return new NdiDriver(c.ndiSource);case'onvif':throw new Error('ONVIF driver is not available yet');
 }
}